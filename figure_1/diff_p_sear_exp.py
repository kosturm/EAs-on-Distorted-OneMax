...
def exp_v1(input):
    ...
    args = []
    for n in n_values:
        args.extend(set_params(algorithms = ["SEAR"],
                               budget = [10000000],
                               target_k = [n**0.4],
                               runs = 50,
                               dim = n,
                               distortion = [np.log(n)],
                               p_distortion = np.around(np.linspace(0.01, 0.3, 40), decimals=4),
                               f_factor = [1.5],
                               s_factor = [1],
                               lam_max = [n*np.log(n)],
...
if __name__ == '__main__':
    ...
    run_parallel(exp_v1([300]), '300_values')
    run_parallel(exp_v1([400]), '400_values')
    run_parallel(exp_v1([500]), '500_values')